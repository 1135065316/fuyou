...
- **房间系统**：
  ...
  - **Godot 房间加载**（`系统/房间管理器.gd`）：从 JSONC 模板实例化地板、装饰物、门、生成点与光照；门系统支持"清怪后开启"
  - **房间生成算法**（`系统/房间生成器.gd` + `系统/走廊生成器.gd`）：随机 DFS 生成 9 层连通房间拓扑，每层 8~15 个房间，含起始房、怪物房、宝箱房、Boss 房；按权重从房间池匹配模板
