...
func 加载房间(模板ID: String, 连通门: Dictionary = {}) -> void:
  ...
  print("[房间管理器] 已加载房间: ", 数据.get("room_name", "未知"))
...
func _设置光照(数据: Dictionary) -> void:
  ...
  var 环境光颜色: Color = 房间工具类.解析颜色(光照配置.get("ambient_color", "#404050"))
  var 环境光强度: float = 光照配置.get("ambient_energy", 0.3)
  ...
  var 环境节点 := WorldEnvironment.new()
  var 环境 := Environment.new()
  环境.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
  环境.ambient_light_color = 环境光颜色
  环境.ambient_light_energy = 环境光强度
  环境节点.environment = 环境
  当前房间.add_child(环境节点)
